...
public class ThreeLegStop extends TwoLaneRural{
	...
	public static Pair<Double[],String[]> conductAnalysis(String[] str, boolean[] bools){
		...
		//builds estimate of annual crashes on road segment
				...
				if (temp<spf) {
					res1.add(spf-temp);
					res2.add("Add " + Integer.parseInt(str[7]) + " number of left approaches");
				}
				temp = spf * numRightCMF(Integer.parseInt(str[8]));
				if (temp<spf) {
					res1.add(spf-temp);
					res2.add("Add " + Integer.parseInt(str[8]) + " number of right approaches");
				}
